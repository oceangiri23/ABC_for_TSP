    ...
    <>
      <nav
        className={`fixed top-2 left-1/2 transform -translate-x-1/2 z-50 w-[90%] max-w-6xl rounded-2xl
                    ${isScrolled ? 'bg-white/80 dark:bg-gray-900/80 backdrop-blur-md outline outline-1 outline-blue-600' : 'bg-blue-100'}`}
      >
        <div className="flex items-center justify-between h-12 px-4">
          {/* Logo */}
          <div className="cursor-pointer" onClick={handleLogoClick}>
            <img src="/assets/SG.png" alt="Logo" className="h-8 w-auto" />
          </div>
          ...
          <div className="hidden lg:flex space-x-5 items-center relative">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={`relative text-sm text-gray-600  hover:text-blue-600 dark:hover:text-blue-400 hover:scale-105 ${isScrolled?"dark:text-gray-300":"dark:text-gray-800"}`}
              >
                {link.label}
